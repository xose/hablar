...
public class SignalsPreferencesPresenter extends PagePresenter<SignalsPreferencesDisplay> implements EditorPage<SignalsPreferencesDisplay> {
	...
	public SignalsPreferencesPresenter(final XmppSession session, final PrivateStorageManager storageManager, final HablarEventBus eventBus,
			final SignalPreferences preferences, final SignalsPreferencesDisplay display, final NotificationManager notificationManager) {
	...
	public boolean hasChanges(final StoredPreferences stored, final SignalPreferences preferences) {
		if ((preferences.incomingMessages != stored.getIncommingMessages()) || (preferences.rosterNotifications != stored.getRosterNotifications())
			...
			if (hasChanges(storedPreferences, preferences)) {
				...
				storageManager.store(storedPreferences, new PrivateStorageResponseEvent.Handler() {
					...
					public void onStorageResponse(PrivateStorageResponseEvent event) {
						IQ parameter = event.getResponseIQ();
						if (!IQ.isSuccess(parameter)) {
							setLoading(true, SignalMessages.msg.saveError());
						}
					}
				});
			}
