...
public class OwnVCardPresenter extends VCardPage implements EditorPage<VCardDisplay> {
	...
	public OwnVCardPresenter(final XmppSession session, final VCardManager vCardManager, final HablarEventBus eventBus, final VCardDisplay display) {
		super(eventBus, display);
		this.session = session;
		this.vCardManager = vCardManager;
		final String title = VCardMessages.msg.ownVCardTitle();
		model.init(IconsBundle.bundle.buddyIconWait(), title, title);
	...
	private void updateVCard(final VCard newVCard) {
		setLoading(true, VCardMessages.msg.updatingOwnVCard());
