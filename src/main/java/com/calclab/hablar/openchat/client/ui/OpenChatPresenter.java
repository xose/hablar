...
public class OpenChatPresenter extends PagePresenter<OpenChatDisplay> {
	...
	public OpenChatPresenter(final XmppSession session, final XmppRoster roster, final PairChatManager chatManager, final HablarEventBus eventBus,
		...
		nameValidator = new TextValidator(display.getNameKeys(), display.getJabberId(), display.getJabberIdError(), display.getAcceptState());
		nameValidator.add(Validators.notEmpty(OpenChatMessages.msg.jabberIdIsEmpty()));
		nameValidator.add(Validators.isValidJid(OpenChatMessages.msg.jabberIdNotValid()));
		nameValidator.add(Validators.notCurrentUser(session, OpenChatMessages.msg.currentUserJidNotAllowed()));
	}
