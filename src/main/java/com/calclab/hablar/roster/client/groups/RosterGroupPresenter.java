...
public class RosterGroupPresenter implements Presenter<RosterGroupDisplay> {
	...
	private final static Comparator<RosterItem> ORDER = RosterItemsOrder.order(RosterItemsOrder.byAvailability, RosterItemsOrder.groupedFirst,
	...
	public RosterGroupPresenter(final RosterGroup group, final Menu<RosterItemPresenter> itemMenu, final RosterGroupDisplay display,
			final RosterConfig rosterConfig, final CommandQueue commandQueue) {
		this.group = group;
	...
	private RosterItemPresenter addRosterItem(final RosterItem item) {
		...
		final RosterItemDisplay itemDisplay = display.newRosterItemDisplay(Idify.id(group.getName()), Idify.id(item.getJID()));

		final RosterItemPresenter presenter = new RosterItemPresenter(group.getName(), itemMenu, itemDisplay, rosterConfig);
