...
public class RosterBasicActions {
	...
	private Action<RosterGroupPresenter> newDeleteGroupAction() {
		return new SimpleAction<RosterGroupPresenter>(RosterMessages.msg.deleteGroupAction(), ID_ACTION_DELETE_GROUP) {
			@Override
			public void execute(final RosterGroupPresenter target) {
				...
				final String title = RosterMessages.msg.confirmDeleteGroupTitle(groupName);
				final String message = RosterMessages.msg.confirmDeleteGroup(groupName, items.size());
	...
	private Action<RosterItemPresenter> newRemoveFromGroupAction() {
		return new SimpleAction<RosterItemPresenter>(RosterMessages.msg.removeFromGroupAction(), ID_ACTION_REMOVE_FROM_GROUP) {
			@Override
	...
	private Action<RosterItemPresenter> newRemoveFromRosterAction() {
		return new SimpleAction<RosterItemPresenter>(RosterMessages.msg.removeContactAction(), ACTION_ID_REMOVE_FROM_ROSTER) {
			@Override
			public void execute(final RosterItemPresenter target) {
	...
	private Action<RosterItemPresenter> newStartChatAction() {
		return new SimpleAction<RosterItemPresenter>(RosterMessages.msg.startChatAction(), ACTION_ID_START_CHAT) {
			@Override
			public void execute(final RosterItemPresenter target) {
