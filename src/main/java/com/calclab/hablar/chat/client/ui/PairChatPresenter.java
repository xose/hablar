...
public class PairChatPresenter extends ChatPresenter implements PairChatPage {
	...
	public void setPresence(final boolean available, final Show show) {
		...
		if (available) {
			display.getState().setText(ChatMessages.msg.stateAvailable(userName));
		} else {
			display.getState().setText(ChatMessages.msg.stateOffline(userName));
		}
	}

	private void fireUserMessage(final String body) {
		final String message = ChatMessages.msg.newChatFrom(userName, TextUtils.ellipsis(body, 25));
