...
public class SearchPage extends PagePresenter<SearchDisplay> {
	...
	public SearchPage(final XmppSession session, final SearchManager searchManager, final Visibility visibility, final boolean closeable,
		...
		model.init(IconsBundle.bundle.searchIcon(), title, title);
	...
	private void search() {
		...
		if (text.length() > 0) {
			...
			display.showMessage(SearchMessages.msg.searchingTerm(text), Level.info);
			...
			searchManager.search(query, new ResultListener<List<SearchResultItem>>() {
				...
				public void onFailure(final String message) {
					display.showMessage(SearchMessages.msg.searchError(), Level.error);
				}
				...
				public void onSuccess(final List<SearchResultItem> items) {
					...
					display.showMessage(SearchMessages.msg.searchResultsFor(text, resultCount), Level.success);
				}
			});
